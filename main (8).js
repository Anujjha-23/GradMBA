...
// ⚠️  Paste your Google Apps Script Web App URL here after deployment
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---- Contact page form → sends to Sheet ---- */
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async e => {
      ...
      const payload = {
        ...
        program: contactForm.querySelector('[name="program"]')?.value?.trim() || '',
        slot:    '',
        page:    'contact.html'
      };
...
function initLeadModal() {
  ...
  function buildCalendar() {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      btn.innerHTML = `<span class="sd-day">${days[d.getDay()]}</span><span class="sd-num">${d.getDate()}</span><span class="sd-mon">${months[d.getMonth()]}</span>`;
  ...
  if (form) {
    form.addEventListener('submit', async function(e) {
      ...
      setTimeout(() => {
        ...
        const slotText = slot
          ? `Your call is booked for ${slot}.`
          : 'Our counsellor will call you within 2 hours.';
